# Deep Learning using time series analysis on Video state dataset
"""
Time-series modeling for YouTube trending 'views' using the first 2000 rows of videostats.csv.
Approach:
1. Load first 2000 rows.
2. Select one ytvideoid with the most records in that slice (so we model a real single-video time series).
   - If that selected video has too few rows (< 60), fall back to using the 'views' column of the whole 2000-row slice.
3. Scale the series with MinMaxScaler.
4. Create sliding-window sequences.
5. Train three models:
   - LSTM (2-layer)
   - GRU (2-layer)
   - Dense (feed-forward baseline on flattened sequences)
6. Compare MAE and RMSE; plot training loss and predictions.
"""
# Imports
...
df = pd.read_csv('Practical–DevelopmentProgrammingBatch_2/videostats.csv', nrows=2000)   
...
def evaluate_and_inverse(model, X_test, y_test, scaler):
    ...
    rmse = mean_squared_error(actual, preds) ** 0.5   # ✅ fixed here
...
plt.figure(figsize=(10,4))
plt.plot(g_actual[:N], label='Actual')
plt.plot(g_preds[:N], label='GRU Pred')
plt.legend()
plt.title('GRU: Actual vs Predicted (first {} points)'.format(N))
plt.show()

plt.figure(figsize=(10,4))
plt.plot(d_actual[:N], label='Actual')
plt.plot(d_preds[:N], label='Dense Pred')
plt.legend()
plt.title('Dense Baseline: Actual vs Predicted (first {} points)'.format(N))
plt.show()

# 11. Save models
lstm.save('/mnt/data/lstm_model.h5')
gru.save('/mnt/data/gru_model.h5')
dense.save('/mnt/data/dense_baseline_model.h5')
print("Saved models to /mnt/data/*.h5")
